...
4. **Маркер версии плагина.** Запиши текущую версию плагина в `.claude/sdx/sdx-version`:
   ```bash
   jq -r .version "${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json" > .claude/sdx/sdx-version
   ```
   Это опорная точка для сверки версии, которую выполняет `/sdx:start` перед стартом каждой новой сессии (см. `sdx/protocol.md`, раздел «Сверка форматов после релиза и миграции»).
5. **Сверка легаси-материала.** Вызови команду `/sdx:reconcile` (без аргументов — интерактивный режим). Структура `docs/` уже развёрнута, а при `--existing` — ещё и реверс-инжинирены `SPEC.md`/`DESIGN.md`; это момент, когда легаси-материал проекта (заметки, TODO, старые сессии) максимально богат, а `docs/backlog/` ещё пуст — оптимальная точка для первого переноса находок. Реконсиляция сама актуализирует `.claude/sdx/sdx-version` по завершении.
6. **CLAUDE.md проекта.** Предложи пользователю добавить в CLAUDE.md проекта краткий SDX-блок (готовый текст: `${CLAUDE_PLUGIN_ROOT}/sdx/templates/claude-md-snippet.md`) — он несёт правило коллизий триады и языковую политику, действующие и вне команд `/sdx:*`. Добавляй ТОЛЬКО с явного согласия пользователя; существующее содержимое CLAUDE.md не изменяй и не удаляй.
7. Запиши отчет об инициализации в `docs/sdx-init-report.md` (включи краткую сводку результата `/sdx:reconcile` из шага 5).
