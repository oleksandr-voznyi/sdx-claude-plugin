...
> **Статус (2026-07-05): worktree-часть пересмотрена ADR-012** — сессия теперь ведётся как
> ветка `sdx/<id>` в основном рабочем дереве, в одном CLI (см.
> `docs/specs/session-inplace-model.md` и ADR-012 в `docs/DECISIONS.md`). Части про
> версионируемые артефакты (REQ-SESS-*), вариант A удаления и автоопределение основной ветки
> (REQ-BRANCH-*, ADR-010) действуют без изменений.
